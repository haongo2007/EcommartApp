...
type ArrowBtnStyleProp = {
  theme?:any,
  showDots?:boolean,
  dot_margin_top?:number|string,
...
const StyledCarouselProvider = styled(CarouselProvider)(({ spacing }:{spacing:number|string}) => ({
  minWidth: 0,
  position: "relative",
...
const StyledSlider = styled(Slider)(({ spacing } : {spacing:number|string}) => ({
  marginLeft: `calc(-1 * ${spacing || "0px"} / 2)`,
  marginRight: `calc(-1 * ${spacing || "0px"} / 2)`,
}));
const StyledDotGroup = styled(DotGroup)(({ dot_margin_top }:{dot_margin_top:number|string}) => ({
  display: "flex",
  justifyContent: "center",
  marginTop: dot_margin_top || "0px",
}));
const StyledDot = styled("div")(({ dot_color, dot_active, theme }:{dot_color?:string, dot_active:number, theme?:any}) => ({
  width: 16,
  height: 16,
  borderRadius: 300,
  margin: "0.25rem",
...
const StyledArrowBackButton = styled(ButtonBack)(({ theme, showArrowOnHover, showDots, dot_margin_top }) => ({
    ...commonArrowBtnStyle({
      theme,
      showDots,
      showArrowOnHover,
...
const StyledArrowNextButton = styled(ButtonNext)(({ theme, showArrowOnHover, showDots, dot_margin_top }) => ({
    ...commonArrowBtnStyle({
      theme,
      showDots,
      showArrowOnHover,
